...
namespace HotelManager
{
    public partial class fBill : Form
    {
        ...
        private void LoadFullBill(DataTable table)
        {
            ...
            txbUser.DataBindings.Add("Text", source, "Name");
        ...
        {
            DataTable dataTable = BillDAO.Instance.LoadTotalBill();

            table.Columns.Add("totalPrice_New", typeof(string));
            table.Columns.Add("finalprice_New", typeof(string));
            for (int i = 0; i < table.Rows.Count; i++)
            {   if(table.Rows[i]["totalPrice"].ToString()!=""&&table.Rows[i]["finalprice"].ToString() !="")
                {
                   /* table.Rows[i]["finalprice_New"] =0.ToString("C0", CultureInfo.CreateSpecificCulture("vi-VN"));
                    table.Rows[i]["totalPrice_New"] = 0.ToString("C0", CultureInfo.CreateSpecificCulture("vi-VN"));*/
                ...
                {
                   table.Rows[i]["discount"] = 0.ToString("0.##%");
                    table.Rows[i]["totalPrice_New"] = ((int)dataTable.Rows[i]["Total"]).ToString("C0", CultureInfo.CreateSpecificCulture("vi-VN"));
                    table.Rows[i]["finalprice_New"]= ((int)dataTable.Rows[i]["Total"]).ToString("C0", CultureInfo.CreateSpecificCulture("vi-VN"));
                }
