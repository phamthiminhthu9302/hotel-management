...
namespace HotelManager
{
    public partial class fServiceType : Form
    {
                    ...
                    else
                    {
                        ...
                        if (check)
                        {
                            ...
                            if (btnCancel.Visible == false)
                            {
                                ...
                                dataGridViewServiceType.SelectedRows[0].Selected = false;
                                dataGridViewServiceType.Rows[index].Selected = true;
                            }
                            else
                            {
                                BtnCancel_Click(null, null);
        ...
        private void btnDeleteServiceTypeClick(object sender, EventArgs e)
        {
            DialogResult result = MessageBox.Show("Bạn có muốn xóa dịch vụ?", "Thông báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1);
            if (result == DialogResult.OK)
                DeleteServiceType();
            comboboxID.Focus();
        ...
        { 
                ServiceType serviceNow = GetServiceTypeNow();
            if (ServiceTypeDAO.Instance.UseServiceinBillForServiceType(serviceNow.Id))
            {
                MessageBox.Show("Không thể xóa.", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                if (ServiceTypeDAO.Instance.UseServiceForServiceType(serviceNow.Id))
                {
                    if (MessageBox.Show("Nếu bạn xóa loại dịch vụ này thì các dịch vụ của nó sẽ bị xóa theo, bạn có muốn tiếp tục?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ServiceDAO.Instance.DeleteServiceByIdServiceType(serviceNow.Id);
                        ServiceTypeDAO.Instance.DeleteServiceType(serviceNow.Id);
                        MessageBox.Show("Xóa thông tin loại dịch vụ thành công!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    if (ServiceTypeDAO.Instance.DeleteServiceType(serviceNow.Id))
                    {
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("Xóa thông tin loại dịch vụ thất bại!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
